...
module SeqTests =
    ...
    module ensureOneTests =
        ...
        [<Fact>]
        let ``ensureOne returns Error with emptyErr when sequence is empty`` () =
        ...
        [<Fact>]
        let ``ensureOne returns Ok with the original sequence when it has exactly one element`` () =
        ...
        [<Fact>]
        let ``ensureOne returns Error with multipleErr when sequence has more than one element`` () =
    ...
    module ensureSizeTests =
        ...
        [<Fact>]
        let ``ensureSize returns Error with tooSmallErr when sequence is smaller than target`` () =
            let target = 3
            let s = seq { 1; 2 }
        ...
        [<Fact>]
        let ``ensureSize returns Ok with the original sequence when length equals target`` () =
            let target = 2
            let s = seq { 10; 20 }
            match Seq.ensureSize target tooSmall tooLarge s with
        ...
        [<Fact>]
        let ``ensureSize returns Error with tooLargeErr when sequence is larger than target`` () =
            let target = 1
            let s = seq { 1; 2; 3 }
            match Seq.ensureSize target tooSmall tooLarge s with
    ...
    module tryGetSingleTests =
        ...
        [<Fact>]
        let ``tryGetSingle returns Error with emptyErr when sequence is empty`` () =
        ...
        [<Fact>]
        let ``tryGetSingle returns Ok with the single element when sequence has exactly one element`` () =
        ...
        [<Fact>]
        let ``tryGetSingle returns Error with multipleErr when sequence has more than one element`` () =
