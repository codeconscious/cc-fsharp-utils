...
module ListTests =
    ...
    module MapFsts =
        ...
        [<Fact>]
        let ``maps elements and preserves second`` () =
            let f = fun x -> x + 1
            let input = [1; 2; 3], "tag"
            let expected = [2; 3; 4], "tag"
            let actual = List.mapFsts f input
            Assert.Equal(expected, actual)

        [<Fact>]
        let ``handles empty list`` () =
            let actual = List.mapFsts (fun (s:string) -> s.Length) ([], 42)
            Assert.Equal(([], 42), actual)

        [<Fact>]
        let ``is generic over types`` () =
            let f = fun (s:string) -> s.ToUpper()
            let input = ["a"; "b"], ('c', 1)
        ...
        [<Fact>]
        let ``does not modify reference of second component`` () =
            let f = fun x -> x * 2
            let xs = [1; 2]
            let y = System.Text.StringBuilder "x"
            let result = List.mapFsts f (xs, y)
            Assert.Same(y, snd result)
            Assert.Equal<list<int>>([2; 4], fst result)

    module MapSnds =

        [<Fact>]
        let ``maps elements and preserves first`` () =
            let f = fun x -> x + 1
            let input = "tag", [1; 2; 3]
            let expected = "tag", [2; 3; 4]
            let actual = List.mapSnds f input
            Assert.Equal(expected, actual)

        [<Fact>]
        let ``handles empty list`` () =
            let actual = List.mapSnds (fun (s:string) -> s.Length) (42, [])
            Assert.Equal((42, []), actual)
        ...
        let ``does not modify reference of second component`` () =
            let f = fun x -> x * 2
            let x = System.Text.StringBuilder "x"
            let ys = [1; 2]
            let result = List.mapSnds f (x, ys)
            Assert.Same(x, fst result)
            Assert.Equal<list<int>>([2; 4], snd result)
